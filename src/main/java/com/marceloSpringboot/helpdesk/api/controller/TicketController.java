...
import com.marceloSpringboot.helpdesk.api.dto.Summary;
...
import com.marceloSpringboot.helpdesk.api.enums.StatusEnum;
import com.marceloSpringboot.helpdesk.api.response.Response;
import com.marceloSpringboot.helpdesk.api.service.TicketService;
...
public class TicketController {
	...
	@PostMapping()
	@PreAuthorize("hasAnyRole('CUSTOMER')")  // Quem cria ticket é apenas o cliente
	...
	public User userFromRequest(HttpServletRequest request) {   // retornar o usuário do token recebido
        String token = request.getHeader("Authorization");  // pegando o token
	...
	public ResponseEntity<Response<Ticket>> update(HttpServletRequest request, @RequestBody Ticket ticket, BindingResult result) {
		...
		try {
			...
			if(ticketCurrent.getAssignedUser() != null) {  // verifica se o ticket pesquisado já foi atribuido para alguem, se sim, setamos o mesmo.
